...
writeFileReq host h localFile remotePath = do
  ...
  let WriteAddress res = fromByteString resp
      writeBlock (port,bid) fblock = do
        unless (bid /= 10) $ putStrLn "Connecting to datanode for block 10"
        handle <- connectTo host (PortNumber $ fromIntegral $ read port)
        hSetBuffering handle NoBuffering
        --hSetBinaryMode handle No
        unless (bid /= 10) $ putStrLn "Writing block id 10"
        B.hPutStrLn handle (toByteString $ CDNWrite bid)
        unless (bid /= 10) $ putStrLn "Sent write message for block 10"
        B.hPut handle fblock
        unless (bid /= 10) $ putStrLn "Sent filedata of block 10"
        hClose handle
